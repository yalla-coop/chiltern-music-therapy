...
const AddSingleContent = ({ state: parentState, actions, decidePath }) => {
  ...
  const goBack = async () => {
    RESET_SINGLE_CONTENT();
    await history.push(navRoutes.THERAPIST.CREATE_PROGRAMME_CONTENT);
  };
            ...
            <Info
              ...
              body={
                <S.Button
                  onClick={() =>
                    history.push(
                      navRoutes.THERAPIST.CREATE_PROGRAMME_CONTENT_HOW_TO_RECORD
                    )
                  }
                >
